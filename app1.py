    ...
    if request.method == 'POST':
        ...
        if password == confirm and 'name' in request.form and 'password' in request.form and 'email' in request.form and 'phone' in request.form and 'confirm' in request.form:
            ...
            if account:
                ...
                # if account is new
                cursor.execute('INSERT INTO users VALUES (%s, %s, %s, %s, %s)', (name, email, phone, password, secure_password))
...
def order():
    ...
    if 'loggedin' in session:
        if request.method == 'POST':
            ...
            cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
            cursor.execute(
                'INSERT INTO orders VALUES (NULL, %s, %s, %s, %s, %s, %s, %s, %s, FALSE, %s, %s, %s, %s, %s)', (
                name, quant, streetadd, streetadd2, city, zip, region, country, session['id'], total, tax, grandtotal,
                datetimenow))
...
def shipping():
    if 'loggedin' in session:
        ...
        if request.method == 'POST':
            ...
            msg.body = "Hello " + userdetails[
                'name'] + "\nYour order for biodiesel is confirmed." + "\n\nShipped to: " + orderdetails[
                           'name'] + "\n\nAddress: " + orderdetails['streetadd'] + ", " + orderdetails[
                           'streetadd2'] + "\n" + orderdetails['city'] + " " + orderdetails['zip'] + "\n" + \
                       orderdetails['region'] + ", " + orderdetails['country'] + "\n\nQuantity: " + str(
                orderdetails['quant']) + " Litres" + "\n" + "Total Ammount Paid (Inclusive of 18% GST): $" + str(
...
def support():
    if 'loggedin' in session:
        ...
        if request.method == 'POST':
            ...
            cursor.execute('INSERT INTO query VALUES (NULL, %s, %s, %s, %s, %s, %s)', (
            session['id'], subject, query, session['name'], userdetails['email'], userdetails['phone'],))
            mysql.connection.commit()
            ...
            msg.body = query + "\n[Sender : " + userdetails['name'] + " | Email : " + userdetails[
                'email'] + " | Phone : " + userdetails['phone'] + " ]"
            mail.send(msg)
